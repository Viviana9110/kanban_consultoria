...
type CompaniesIntent = { kind: 'create-company' } | { kind: 'create-diagnostic' } | { kind: 'open-company'; companyId: string } | { kind: 'open-first-diagnostic' }
type DetailIntent = { kind: 'create-diagnostic' } | { kind: 'open-first-diagnostic' }
...
function Companies({ user, intent, onConsumeIntent }: { user: User; intent: CompaniesIntent | null; onConsumeIntent: () => void }) {
  ...
  const applyIntent = useCallback((available: Company[], pending: CompaniesIntent | null) => {
    ...
    const target = pending.kind === 'open-company' ? available.find((item) => item.id === pending.companyId) : available[0]
